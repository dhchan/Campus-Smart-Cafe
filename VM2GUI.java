...
public class VM2GUI extends JFrame {
	...
	public VM2GUI(Card card) {
		...
		JLabel lblCafe1 = new JLabel("Vending Machine 2");
		...
		for (int i = 0; i < vm2.getMenu().size(); i++) {
			String name = vm2.getMenu().get(i).getName();
			double price = vm2.getMenu().get(i).getPrice();
			int quantity = vm2.getMenu().get(i).getQuantity();
			modelMenu.addRow(new Object[] { name, price, quantity });
		}
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (menuTable.getSelectedRowCount() != 0) {
					if (modelSC.getRowCount() == 0) {
						if ((int) (menuTable.getValueAt(menuTable.getSelectedRow(), 2)) >= Integer
								.parseInt(quantity_comboBox.getSelectedItem().toString())) {
							...
							display_textPane.setText(
									"The Vending Machine Doesn't Have Enough Quantity for Your Selected Item!");
						}
					} else {
						...
						if (scMap.containsKey((String) menuTable.getValueAt(menuTable.getSelectedRow(), 0))) {
							...
							if (quantity_new > (int) menuTable.getValueAt(menuTable.getSelectedRow(), 2)) {
								display_textPane.setText(
										"The Vending Machine Doesn't Have Enough Quantity for Your Selected Item!");
							} else {
		...
		modelSC.addTableModelListener(new TableModelListener() {
			public void tableChanged(TableModelEvent e) {
				...
				for (int i = 0; i < modelSC.getRowCount(); i++) {
					...
					for (int j = 0; j < vm2.getMenu().size(); j++) {
						if (vm2.getMenu().get(j).getName().equals(foodName)) {
							sum = (float) (sum + vm2.getMenu().get(j).getPrice()
									* Float.parseFloat(shoppingCartTable.getValueAt(i, 1).toString()));
							cal_suminsc = cal_suminsc + vm2.getMenu().get(j).getCalories()
									* Float.parseFloat(shoppingCartTable.getValueAt(i, 1).toString());
						}
					}
				}
				...
				if (shoppingCartTable.getRowCount() > 0 && !totalCost_textField.getText().equals("0.0")) {
					if (card1.getExpenseProfile().getTotalFunds() >= Double
						...
						if (reply == JOptionPane.YES_OPTION) {
							...
							// update Vending Machine menu quantity!
							for (int i = 0; i < shoppingCartTable.getRowCount(); i++) {
								for (int j = 0; j < vm2.getMenu().size(); j++) {
									if (vm2.getMenu().get(j).getName()
											.equals((String) shoppingCartTable.getValueAt(i, 0))) {
										vm2.getMenu().get(j).setQuantity(vm2.getMenu().get(j).getQuantity()
												- Integer.parseInt(shoppingCartTable.getValueAt(i, 1).toString()));
										modelMenu.setValueAt(vm2.getMenu().get(j).getQuantity(), j, 2);
									}
								}
							}
							vm2.save();
	...
	class ExpenseProfileDisplay extends PApplet {
		...
		public void pieChart(float diameter) {
			...
			arc(210, 100, diameter, diameter, lastAng, lastAng + radians(
					(float) (card1.getExpenseProfile().getFundsSpentToday() * 360 / Float.parseFloat(totalfund_textField.getText()))));
			lastAng += radians(
					(float) (card1.getExpenseProfile().getFundsSpentToday() * 360 / Float.parseFloat(totalfund_textField.getText())));
			
			fill(200);
			arc(210, 100, diameter, diameter, lastAng, lastAng + radians(
					(float) (card1.getExpenseProfile().getTotalFunds() * 360 / Float.parseFloat(totalfund_textField.getText()))));
			lastAng += radians(
					(float) (card1.getExpenseProfile().getFundsSpentToday() * 360 / Float.parseFloat(totalfund_textField.getText())));
		...
		public void pieLegend() {
			...
			fill(colorList[expenseProfileData.size()][0], colorList[expenseProfileData.size()][1], colorList[expenseProfileData.size()][2]);
			...
			textAlign(LEFT, CENTER);
			textSize(11);
			text("Unused", nameList.get(expenseProfileData.size()+1).x, nameList.get(expenseProfileData.size()+1).y);

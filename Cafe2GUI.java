...
public class Cafe2GUI extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					Cafe2GUI frame = new Cafe2GUI(card1);
	...
	public Cafe2GUI(Card card) {
		...
		JLabel lblCafe1 = new JLabel("Cafe 2");
		...
		for (int i = 0; i < cafe2.getMenu().size(); i++) {
			String name = cafe2.getMenu().get(i).getName();
			double price = cafe2.getMenu().get(i).getPrice();
		...
		modelSC.addTableModelListener(new TableModelListener() {
			public void tableChanged(TableModelEvent e) {
				...
				for (int i = 0; i < modelSC.getRowCount(); i++) {
					...
					for (int j = 0; j < cafe2.getMenu().size(); j++) {
						if (cafe2.getMenu().get(j).getName().equals(foodName)) {
							sum = (float) (sum + cafe2.getMenu().get(j).getPrice()
									* Float.parseFloat(shoppingCartTable.getValueAt(i, 1).toString()));
							cal_suminsc = cal_suminsc + cafe2.getMenu().get(j).getCalories()
									* Float.parseFloat(shoppingCartTable.getValueAt(i, 1).toString());
						}
					}
				}

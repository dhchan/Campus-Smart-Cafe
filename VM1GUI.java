...
public class VM1GUI extends JFrame {
	...
	public VM1GUI(Card card) {
		...
		JLabel lblCafe1 = new JLabel("Vending Machine 1");
		...
		for (int i = 0; i < vm1.getMenu().size(); i++) {
			String name = vm1.getMenu().get(i).getName();
			double price = vm1.getMenu().get(i).getPrice();
			int quantity = vm1.getMenu().get(i).getQuantity();
		...
		modelSC.addTableModelListener(new TableModelListener() {
			public void tableChanged(TableModelEvent e) {
				...
				for (int i = 0; i < modelSC.getRowCount(); i++) {
					...
					for (int j = 0; j < vm1.getMenu().size(); j++) {
						if (vm1.getMenu().get(j).getName().equals(foodName)) {
							sum = (float) (sum + vm1.getMenu().get(j).getPrice()
									* Float.parseFloat(shoppingCartTable.getValueAt(i, 1).toString()));
							cal_suminsc = cal_suminsc + vm1.getMenu().get(j).getCalories()
									* Float.parseFloat(shoppingCartTable.getValueAt(i, 1).toString());
						}
					}
				}
				...
				if (shoppingCartTable.getRowCount() > 0 && !totalCost_textField.getText().equals("0.0")) {
					if (card1.getExpenseProfile().getTotalFunds() >= Double
						...
						if (reply == JOptionPane.YES_OPTION) {
							...
							for (int i = 0; i < shoppingCartTable.getRowCount(); i++) {
								for (int j = 0; j < vm1.getMenu().size(); j++) {
									if (vm1.getMenu().get(j).getName()
											.equals((String) shoppingCartTable.getValueAt(i, 0))) {
										vm1.getMenu().get(j).setQuantity(vm1.getMenu().get(j).getQuantity()
												- Integer.parseInt(shoppingCartTable.getValueAt(i, 1).toString()));
										modelMenu.setValueAt(vm1.getMenu().get(j).getQuantity(), j, 2);
									}
								}
							}
							vm1.save();
